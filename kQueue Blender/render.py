...
""".strip()
            ...
            if not self.listener_started:
                ...
                self.listen_worker.gProgressBar.connect(mw.w_gProgressBar.setValue)
                self.listen_worker.pProgressBar.connect(mw.w_pProgressBar.setValue)
                self.listen_worker.rProgressBar.connect(mw.w_rProgressBar.setValue)
...
class RenderListenWorker(qtc.QThread):
    finished = qtc.pyqtSignal()
    gProgressBar = qtc.pyqtSignal(int)
    pProgressBar = qtc.pyqtSignal(int)
    rProgressBar = qtc.pyqtSignal(int)
        ...
        while True:
            ...
            try:
                ...
                for line in io.TextIOWrapper(preset.process.stdout, encoding="utf-8"):
                    ...
                    mw.w_gProgressETA.setText(f"Elapsed: {h:02.0f}:{m:02.0f}:{s:02.0f} | AVG: {avg_h:02.0f}:{avg_m:02.0f}:{avg_s:02.0f} | ETA: {eta_h:02.0f}:{eta_m:02.0f}:{eta_s:02.0f}")
...
""")
                    ...
                    if found:
                        ...
                        self.rProgressBar.emit(round(progress * 100))
                        continue
                    ...
                    if found:
                        ...
                        self.rProgressBar.emit(100)
                        self.pProgressBar.emit(100)
                        mw.w_gProgress.setText(f'{preset.global_frame}/{preset.global_frames}')
                        mw.w_pProgress.setText(f'{preset.project_frame}/{preset.project_frames}')
                        continue
                    ...
                    if found:
                        ...
                        if preset.frame_flag != preset.last_frame_flag:
                            ...
                            self.gProgressBar.emit(round(gProgress * 100))
                            self.pProgressBar.emit(round(pProgress * 100))
                            mw.w_gProgress.setText(f'{max(0, preset.global_frame - 1)}/{preset.global_frames}')
                            mw.w_pProgress.setText(f'{max(0, preset.project_frame - 1)}/{preset.project_frames}')
